...
High Performance Computing(HPC) 클러스터는 대규모 애플리케이션에 최고의 컴퓨팅 성능을 제공하기 위해 생성됩니다. 이렇듯 많은양의 데이터를 처리해야할 경우가 많습니다. 꽤 오랜 시간 동안 프로세서와 메모리의 속도는 급격히 증가했지만 I/O 시스템의 성능은 이보다 뒤쳐지고 있습니다. 따라서 상대적으로 성능이 떨어지는 I/O 성능은 클러스터의 전체 성능을 저하 시킬 수 있습니다. 이를 위해  `lustre File System`을 사용합니다.
